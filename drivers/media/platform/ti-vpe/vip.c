...
static int vip_setup_parser(struct vip_port *port)
{
	...
	config0 &= ~(VIP_PIXCLK_EDGE_POLARITY | VIP_HSYNC_POLARITY | VIP_VSYNC_POLARITY);
	config0 |= VIP_PIXCLK_EDGE_POLARITY | VIP_VSYNC_POLARITY;

	config0 |= ((sync_type & VIP_SYNC_TYPE_MASK) << VIP_SYNC_TYPE_SHFT);
	
	/*printk("!!! kb-oberon !!! %s:%s:%d: config0=%x flags=%x\n", __FILE__, __FUNCTION__, __LINE__, config0, flags);*/
