...
static int _init_clocks(struct omap_hwmod *oh, void *data)
{
	...
	//printk("!!!!! %s: %s: %d: oh->_state=%u\n", __FILE__, __FUNCTION__, __LINE__, oh->_state);
...
static int _enable(struct omap_hwmod *oh)
{
	...
	//printk("!!!!! %s: %s: %d: start %s _int_flags=%x\n", 
	//		__FILE__, __FUNCTION__, __LINE__, oh->name, oh->_int_flags);
	/*
				    ...
				    int index, struct device_node *np)
{
	...
	printk("omap_hwmod: %s: MPU register target at va %p\n",
		 oh->name, va_start);
