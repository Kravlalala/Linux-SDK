				...
				const struct i2c_device_id *id)
{
	...
	/*_CONFIG_KB_OBERON_*/
	adv7393_write(&state->sd, ADV7393_SOFT_RESET, 	   	0x02);
	adv7393_write(&state->sd, ADV7393_POWER_MODE_REG,  	0x1C);
	adv7393_write(&state->sd, ADV7393_MODE_SELECT_REG, 	0x00);
	adv7393_write(&state->sd, ADV7393_SD_MODE_REG1,	   	0x11);
	adv7393_write(&state->sd, ADV7393_SD_MODE_REG2,   	0xC3);
	adv7393_write(&state->sd, ADV7393_FSC_REG0,		0xCB);
	adv7393_write(&state->sd, ADV7393_FSC_REG1,    		0x8A);
	adv7393_write(&state->sd, ADV7393_FSC_REG2,    		0x09);
	adv7393_write(&state->sd, ADV7393_FSC_REG3,    		0x2A);
